          ...
          {voicesLoading ? (
            ...
            <div className="space-y-4">
              ...
              {voices?.some((v) => (v as unknown as { provider?: string }).provider === "google") && (
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Google Cloud TTS</p>
                  {voices?.filter((v) => (v as unknown as { provider?: string }).provider === "google").map((voice) => (
                    <button
                      key={voice.id}
                      onClick={() => setSelectedVoiceId(voice.id)}
                      className={`w-full text-left p-3 rounded-lg border transition-colors ${
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`h-8 w-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                          selectedVoiceId === voice.id ? "bg-green-100 dark:bg-green-900/30" : "bg-muted"
                        }`}>
                          <Mic2 className={`h-4 w-4 ${selectedVoiceId === voice.id ? "text-green-600" : "text-muted-foreground"}`} />
                        </div>
                        <div className="min-w-0">
                          <p className="font-medium text-sm">{voice.name}</p>
                          {voice.description && (
